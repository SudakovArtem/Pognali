...
return function(media){return{matches:styleMedia.matchMedium(media||"all"),media:media||"all"};};}());(function(w,doc,image){"use strict";function expose(picturefill){if((typeof module==="undefined"?"undefined":_typeof(module))==="object"&&_typeof(module.exports)==="object"){module.exports=picturefill;}else if(typeof define==="function"&&define.amd){define("picturefill",function(){return picturefill;});}
if(_typeof(w)==="object"){w.picturefill=picturefill;}}
...
return candidates;};pf.backfaceVisibilityFix=function(picImg){var style=picImg.style||{},WebkitBackfaceVisibility=("webkitBackfaceVisibility"in style),currentZoom=style.zoom;if(WebkitBackfaceVisibility){style.zoom=".999";WebkitBackfaceVisibility=picImg.offsetWidth;style.zoom=currentZoom;}};pf.setIntrinsicSize=function(){var urlCache={};var setSize=function setSize(picImg,width,res){if(width){picImg.setAttribute("width",parseInt(width/res,10));}};return function(picImg,bestCandidate){var img;if(!picImg[pf.ns]||w.pfStopIntrinsicSize){return;}
...
if(source.getAttribute("src")!==null&&(typeof console==="undefined"?"undefined":_typeof(console))!==undefined){console.warn("The `src` attribute is invalid on `picture` `source` element; instead, use `srcset`.");}
